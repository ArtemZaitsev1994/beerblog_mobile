...
class _BeerListState extends State<BeerList>
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: TabBarView(
          controller: _tabController,
          children: <Widget>[_firstScreen(), _secondScreen()],
        ));
  }
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            if (snapshot.data['user'] != null) {
              ...
              return _addNewBeerScreen(snapshot.data['user']);
            } else {
  ...
  Widget _firstScreen() {
    return FutureBuilder(
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            user = snapshot.data['user'];
            return _firstScreen1();
          }
          return Center(child: CircularProgressIndicator());
        });
  }

  Widget _firstScreen1() {
    return StreamBuilder<String>(
        stream: sorting.changedSorting,
        builder: (BuildContext context, snap) {
          return FutureBuilder<BeerData>(
              future: getBeerList(),
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  return Column(children: <Widget>[
                      ...
                      child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            return Card(
                              ...
                              child: ListTile(
                                  title:
                                      Text('${snapshot.data.beer[index].name}'),
                                  leading: Image.network(
                                      '${snapshot.data.beer[index].mini_avatar}'),
                                  trailing: Row(
                                    mainAxisSize: MainAxisSize.min,
                                    children: <Widget>[
                                      Icon(Icons.star, color: Colors.yellow),
                                      Text(snapshot.data.beer[index].rate
                                          .toString())
                                    ],
                                  )),
                              onTap: () {
                                Navigator.pushNamed(context, "/beer_item",
                                    arguments:
                                        snapshot.data.beer[index].beerId);
                              },
                            ));
                          }),
                    ),
                    Expanded(
                        ...
                        child: Container(
                            ...
                            child: Row(
                                ...
                                children:
                                    makePagination(snapshot.data.pagination))))
                  ]);
                } else if (snapshot.hasError) {
                  return Text('Error');
                }
  ...
  Widget _addNewBeerScreen(_user) {
    user = _user;
  ...
  Future<BeerData> getBeerList() async {
    var sortMap = {
      "Новые": "time_desc",
      "Старые": "time_asc",
      "Худшие": "rate_asc",
    ...
    if (response.statusCode == 200) {
      return BeerData.fromJson(json.decode(utf8.decode(response.bodyBytes)));
    }
